...
func (srv *TgService) CQ_subscribe(m models.Update) error {
	...
	mess := fmt.Sprintf("Так держать! Выполнив следующее условие, ты обходишь 97%% участников.\n\nУсловие второе:\nВыложи себе в инстаграм stories нашу картинку и в соответствующем поле отметь инстаграм-аккаунт раздачи: %s %s. \nКак закончишь, жми ☑️ Отметил\n\nИли:\nПригласи двух друзей по своей уникальной ссылке: реферальная.ссылка/пользователя. Отправь ссылку друзьям. Когда они вступят по ней, нажми ☑️ Пригласил\n\nP.S. Узнать кто и сколько человек вступили по твоей ссылке можно, нажав кнопку 'Мои рефералы'.", "@mrgeniuz1", srv.ChInfoToLinkHTML("https://www.instagram.com/mrgeniuz1", "(прямая ссылка на профиль)"))
	...
	_, err = srv.SendDocumentWCaption(fromId, text, fileNameInServer)
	if err != nil {
		return fmt.Errorf("CQ_subscribe SendDocumentWCaption err: %v", err)
	}
